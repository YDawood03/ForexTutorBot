...
**The application is live and deployed on Microsoft Azure!**

**Try it here:** [https://forextutorbot-app-c5cmhze9dbhed0cq.southafricanorth-01.azurewebsites.net/](https://forextutorbot-app-c5cmhze9dbhed0cq.southafricanorth-01.azurewebsites.net/)
...
* **Cloud Hosting:** Microsoft Azure App Service / GitHub Actions (CI/CD)

---

## How It Works Under the Hood

1. **Vectorization:** Upon application startup, the educational knowledge base is parsed, and the questions are converted into mathematical arrays using a TF-IDF Vectorizer to filter out English stop words.
2. **Query Intake:** When a user submits a query via the UI, the string is processed and transformed into the same vector space.
3. **Similarity Analysis:** A cosine similarity matrix calculation runs instantly between the user input vector and the pre-computed matrix to isolate the maximum coordinate index.
4. **Validation:** If the resulting score clears the 0.25 mathematical threshold boundary, the corresponding curriculum value is returned; otherwise, a helpful system validation error displays.
...
This application is continuously deployed to **Microsoft Azure App Service** via **GitHub Actions**, ensuring that the latest version is always live.
